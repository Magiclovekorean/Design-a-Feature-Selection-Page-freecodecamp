...
### User stories:
1. [x] Your page should have an `h1` element with the text `Feature Selection`.
2. [x] Your page should have a `div` element with the class `feature-card-container.`
3. [x] Your page should have at least two `label` elements each with the class `feature-card` inside the `div`.
4. [x] Each of your `label` elements should have label text and an `input` element of type `checkbox` in them.
5. Create a selector that targets the checkboxes, and apply the following styling:
    - [x] All of your checkbox elements should be set to `appearance: none;` in your CSS.
